...
ok(/600, _agentSignal\(ctx\), 'secondary',[^\n]*agent_anomaly_scan/.test(mode), 'T2 冷门二分类下 secondary 且传递统一取消信号');
ok(/2800, _agentSignal\(ctx\), 'secondary'\);\s+\/\* T2/.test(mode), 'T2 动作脚手架下 secondary 且传递统一取消信号');
...
ok(/async function _reaskJSON\(gm, msgs, maxTok, tier, signal\)/.test(depth) && /Math\.round\(maxTok \* 1\.5\)/.test(depth) && /signal \|\| null, tier/.test(depth), 'T4 重问一次·输出上限×1.5 且保留逐调用取消信号');
...
ok(/promptTrim: state\.promptTrim \|\| null/.test(mode) && /jsonReasks: gm\._agentJsonReasks \|\| 0/.test(mode) && /durMs: state\.t0/.test(mode), 'T6 meta 收 T1 收敛痕/T4 重问数/整回合耗时');
