...
vm.runInContext(fs.readFileSync(path.join(ROOT, 'tm-endturn-render.js'), 'utf8'), sandbox, { filename: 'tm-endturn-render.js' });

assert(typeof sandbox._coreMetricReasons === 'function' && typeof sandbox._popReasonHtml === 'function' && typeof sandbox._rucReasonChips === 'function', '① 三 helper 可调(顶层函数)');

// ── B2 皇威:按 label 匹配 turnChanges.variables 已有 reasons ──
const hw = sandbox._coreMetricReasons('huangwei', '皇威');
...
const mx = sandbox._coreMetricReasons('minxin', '民心');
...
assert(sandbox._coreMetricReasons('someMetric', '不存在').length === 0, '⑤ 无匹配→空数组(不误挂)');

// ── _rucReasonChips 把逐因渲成 chip(端到端·民心因果真上屏) ──
const chips = sandbox._rucReasonChips(mx);
...
const fleeHtml = sandbox._popReasonHtml(['flee'], '赋役、灾荒、兼并所迫');
...
const deathHtml = sandbox._popReasonHtml(['death'], '新生·死亡·逃散');
assert(/新生·死亡·逃散/.test(deathHtml) && /tr-reason-txt/.test(deathHtml), '⑨ 无真因→回落诚实通用词');

// ── 静态契约 ──
const src = fs.readFileSync(path.join(ROOT, 'tm-endturn-render.js'), 'utf8');
assert(/reasons: _coreMetricReasons\(k, label\)/.test(src), '⑩ 政治核心 push 已接 _coreMetricReasons(非 reasons:[])');
assert(/reasonsHtml: _popReasonHtml\(\['flee'\]/.test(src), '⑪ 逃户 reasonsHtml 已接 _popReasonHtml');
