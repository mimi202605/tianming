...
// 幽灵键闸 + dry-run（2026-07-10·hardChange 档拒创建新根·absolute allowCreate 放行）
assert(context._wtApplyHardChange('灵气', 'set', 999) === false, 'unknown root should be rejected (ghost-key gate)');
assert(!Object.prototype.hasOwnProperty.call(context.GM, '灵气'), 'rejected path should not create GM.灵气');
assert(context._wtApplyHardChange('人物.不存在的人.loyalty', 'set', 100) === false, 'unknown character should be rejected');
assert(!Object.prototype.hasOwnProperty.call(context.GM.chars, '不存在的人'), 'rejected char path should not shadow chars array');
assert(context._wtApplyHardChange('灵气', 'set', 7, { allowCreate: true }) === true, 'absolute allowCreate should permit new root');
assert(context.GM['灵气'] === 7, 'allowCreate should write the new root');
const dry1 = context._wtDryRunHardChange('人物.袁崇焕.忠诚');
assert(dry1.ok === true && dry1.kind === 'char', 'dry-run should resolve existing character');
const dry2 = context._wtDryRunHardChange('人物.岳飞.忠诚');
assert(dry2.ok === false && dry2.kind === 'ghost' && /岳飞/.test(dry2.reason), 'dry-run should reject unknown character with named reason');
const dry3 = context._wtDryRunHardChange('精力');
assert(dry3.ok === true, 'dry-run should accept canonical _energy alias');
const dry4 = context._wtDryRunHardChange('仙丹数量');
assert(dry4.ok === false && dry4.kind === 'ghost', 'dry-run should flag ghost-key path');
const dry5 = context._wtDryRunHardChange('仙丹数量', { allowCreate: true });
assert(dry5.ok === true && dry5.kind === 'generic', 'dry-run allowCreate should mirror absolute freedom');
