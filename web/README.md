...
**在线游玩** —— 直接打开 <https://misfit-user.github.io/tianming/>，在设置中填入你的 AI API key 即可开局。公开站点只跟随仓主明确发布的 `ship-*` Release（或手动指定 ref），不会自动发布 `main` 开发态。
...
- 游戏的主体内容由 Web 运行时承载，可通过热更新滚动到最新公开内容版（当前 1.3.4.8）；
...
`web/` 是天命 Web 运行时，也是 Pages 的构建输入；Pages workflow 会先生成官方剧本派生物并通过发布树闸，再部署受检 staging。
