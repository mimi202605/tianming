...
## 五½、拍板记录（2026-07-11 owner 已答·三项全取推荐档）

1. **默认口径统一 → 一律据奏当默认**：严格史实+开关下所有面板标题数字=据奏值+「据奏」徽；实情须揭，已揭键显实情并附对照。骗就骗彻底。
2. **顶栏掉头 → 标题直接换据奏**：真值从顶栏消失、须揭才见（民心/吏治 perceivedIndex 掉头照此）。
3. **军额虚增源 → 军队吏治面 + 统帅忠诚点**：面=corruption.subDepts.military·点=该军统帅忠诚（直臣治军虚额少、离心将领吃饷狠）。

★施工避让（2026-07-11）：tm-var-drawers.js（帑廪/民心/吏治抽屉补充段）是并行会话在飞脏文件——S2/S3 波先做顶栏+右rail+专属 panel，抽屉段待其落账后补接。

## 五¾、落地记录（2026-07-11）

- S1 引擎=27fc901f · S2 财政三面=cfb58282 · S3a 顶栏民心/吏治掉头=fa880d93 · **S3b-S7 铺面=本次**：
  舆图 mood/office 打分器+势力tip（民变/战区/缺员等公开事实不失真·tax层名义vs实征真账不套）、
  纲纪阶层满意度三处、顶栏户口 national.* 全 bad、户口志 hover 收严+役政志对照行翻转、
  军额据奏名员（military 分部门吏治面×统帅忠诚点·核饷点验揭实额）、
  揭真三通道（核饷 hexiang/查案 audit=吏治+民情全局+当地三键/门生密报 mishu=当地役政+民情）、
  奏疏 AI 据奏口径（税压/危机省先据奏再过阈值=「该奏不奏」壅蔽涌现+口径指令）。
- **未接面（记债）**：tm-var-drawers.js 民心/帑廪/吏治抽屉段（并行会话在飞·落账后补接）；GM.vars 剧本自定义变量喂奏疏（语义方向未知暂真值）；诏狱拷问揭真（需囚犯-议题映射）；classPressure 层（桥账本已有 true/perceived 双轨）；renli.reported 与 RV 两套产线毫厘差长期归并。
