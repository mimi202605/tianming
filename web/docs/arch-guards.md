...
★codemod 军规（第二拆之鉴）：**必须幂等**——重跑曾把三连 load 再展开成五连/read 串接嵌套·
同 context 顶层 const 重声明=SyntaxError。护栏=目标文件已含分片名即 skip。装载形匹配须含
`path.join|path.resolve` 两形（首轮漏 resolve·全量 sweep 抓获）。
巨石分诊三类：①顶层函数型→本范式；②IIFE 包裹型(endturn-ai/content-manager/phase8族等)→
alias 范式(见记忆 feedback_large_file_split_paradigm)；③单函数巨石(tm-endturn-apply 的
writeBack=125..5880 一根函数·§1-10 全内联段)→切割即重构·须专项设计勿套本范式；
tm-endturn-prompt=范式反例(prompt 拼接型)不拆。
队列（顶层函数型·可直接套）：tm-keju-runtime 3780·tm-wendui 3517·tm-npc-decision 3474·tm-patches 3389。
