...
- **2026-07-18 管线更新**：旧 runtime snapshot 与 `SNAP_QS` 已退役。唯一真源为仓根 `scenarios/*（官方）.json`；修改后运行 `node web/scripts/sync-official-scenarios.js`，由 manifest 在玩家选中剧本时按需加载紧凑完整脚本。禁止反向修改 `web/scenarios/*.js` 或各 bundle。
