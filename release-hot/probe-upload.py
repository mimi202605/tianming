...
host = '64.83.47.115'; port = 2222; user = 'root'; pw = os.environ.get('TIANMING_SSH_PASS')
if not pw:
    print('NO TIANMING_SSH_PASS env'); sys.exit(1)
c = paramiko.SSHClient(); c.set_missing_host_key_policy(paramiko.AutoAddPolicy())
try:
    c.connect(host, port=port, username=user, password=pw, timeout=25)
    print('CONNECTED', host, port)
    serve = '/opt/1panel/apps/openresty/openresty/www/sites/api.themisfitserspeople.top/index/tianming/hot'
    for d in [serve, '/tmp']:
