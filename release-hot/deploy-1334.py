#!/usr/bin/env python3
"""Server-side deploy for Tianming 1.3.3.4 — runs ON the server (python3), pulls from the
GitHub Release over HTTP (no SSH from dev side). MEMORY-SAFE: streams big zips to disk in
chunks (never reads a whole 4MB/516MB file into RAM — small VPS would OOM). Handles both
ends + 邸报:
  - 电脑端 Electron 热更: full-zip -> sha-addressed /hot/files/ + /hot/manifests/1.3.3.4.json
    (incremental·1.3.3.3 玩家只下变动文件) + 整包 + hot-latest.json
  - 安卓 Capgo 热更: /capgo/bundles/1.3.3.4.zip + /capgo/latest.json
  - 邸报 standalone: /changelog.json (FROM THE DESKTOP ZIP)

Run on the server (1Panel VNC / web terminal):
  curl -sL https://github.com/misfit-user/tianming/releases/download/ship-1.3.3.4/deploy-1334.py -o /tmp/d.py
  python3 /tmp/d.py
"""
...
def main():
    ...
    print(f"[deploy {VER}] 下载电脑端热更整包 (~4MB·流式·落真实磁盘) ...", flush=True)
    zp = f"{REAL}/{ZIP_NAME}"
    zpath = zp + ".new"
    ...
    hotlatest = parse_json_bytes(fetch_small(f"{REL}/hot-latest.json"))
    ...
    # ===== 安卓 Capgo 热更 (516MB·流式直落·全程不进内存) =====
    print(f"[deploy {VER}] 下载安卓 Capgo bundle (~516MB·流式) ...", flush=True)
    os.makedirs(CAPGO_BUNDLES, exist_ok=True)
    capgo_latest = parse_json_bytes(fetch_small(f"{REL}/latest.json"))
    ...
    print(f"=== DONE 1.3.3.4 双端热更 + 邸报 全部发布 ===")
