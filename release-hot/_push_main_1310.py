#!/usr/bin/env python3
# Push 1.3.1.0 变更文件到 GitHub main (Path B: GraphQL createCommitOnBranch·不 clone·repo 597MB clone 不可行)
# 文件列表 = delta 包内的变更/新增文件(132 个·代码+changelog+README+docs·不含 assets)
...
DELTA = Path("C:/Users/37814/Desktop/tianming/release-hot/tianming-hot-1.3.1.0-delta.zip")
MAX_RAW = 900 * 1024  # 0.9MB raw/batch -> base64 ~1.2MB·小 payload 抗弱网 499/EOF

def gh_query(query, variables, retries=7):
    payload = {"query": query, "variables": variables}
    tmp = Path(os.environ.get("TEMP", "C:/Users/37814/AppData/Local/Temp")) / "gh-payload-1310.json"
...
# 1. 变更文件列表(README.md 单独保证纳入)
dz = zipfile.ZipFile(DELTA)
paths = set(n for n in dz.namelist() if n != "manifest.json")
paths.add("README.md")  # README 也推(本版更新了·但不在热更 delta 里)
paths = sorted(paths)
...
for i, batch in enumerate(batches, 1):
    ...
    var = {"input": {
        ...
        "message": {"headline": f"release 1.3.1.0 part {i}/{len(batches)}: 史册库 + AI记忆引擎 + 国师助手 + 纪事补结论",
